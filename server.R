...
shinyServer(function(input, output, session) {
    ...
    X <- reactive({
        my.rv <- switch(input$rv,
                        "be" = make.RV(c(0, 1), function(x, p = input$beparam1) p^x * (1 - p)^(1 - x), range = TRUE),     
                        "bn" = make.RV(c(0, input$bnparam1), function(x, n=input$bnparam1, p = input$bnparam2) choose(n, x) * p^x * (1 - p)^(n - x), range = TRUE),
                        "du" = make.RV(c(input$duparam1, input$duparam2), function(x, a=input$duparam1, b=input$duparam2){1 / (b - a + 1)}, range = TRUE),
                        "gm" = make.RV(c(1, Inf), function(x, p=input$gmparam1) (1 - p)^(x - 1) * p, range = TRUE),
                        "hg" = make.RV(c(max(0, input$hgparam3 + input$hgparam2 - input$hgparam1), min(input$hgparam2, input$hgparam3)), function(x, N = input$hgparam1, k = input$hgparam2, n = input$hgparam3) choose(k, x) * choose(N - k, n - x) / choose(N, n), range = TRUE),
                        "nb" = make.RV(c(0, Inf), function(x, r = input$nbparam1, p = input$nbparam2) choose(x + r - 1, x) * (1 - p)^r * p^x, range = TRUE),
                        "pois" = make.RV(c(0, Inf), function(x, lambda = input$poisparam) { lambda^x * exp(-lambda) / factorial(x) }, range = TRUE)
        )
